---
title: 登录到服务器-Almalinux
author: Comardom
description: 记录 AlmaLinux 服务器的 SSH 登录、安全组与端口配置、用户权限划分和基础环境准备流程。
pubDate: 2026-08-08
draft: false
---
# 登录与环境基础配置
...
2. 在终端中使用ssh登录。对于服务器管理的一般情况，ssh你只需要知道两个语法：ssh -i和ssh -p，也就是列出来的三种写法。
   ...
   ```
   是等价的。
3. 系统升级。此处以AlmaLinux作为例子，我们使用dnf包管理。建议每周都执行一次，检查有没有需要更新的东西。你需要执行这些步骤：

   ```bash
   # [root]
   sudo dnf update -y
   dnf clean all # 清理所有缓存（包、元数据）
   dnf clean packages # 仅清理已下载的 RPM 包
   dnf clean metadata # 仅清理仓库元数据（下次需重新下载）
   ```
4. 添加自定义仓库
   ```bash
   # [root]
   sudo dnf install epel-release -y # 自动配置 EPEL 仓库
   ```
5. 安装必须要用到的软件

   ```bash
   # [root]
   sudo dnf install vim git zip unzip tar -y
   # 卸载软件以后清理系统
   sudo dnf autoremove
   ```
...
7. 
   ...
   root用户是用于系统级维护的，一开始登录服务器是要用到的，后面就会禁用。创建用户组是为了便于统一管理，方便检查权限。
   ...
   ```bash
   # [root]
   sudo usermod -aG wheel adminAnsible  # 用户加入用户组
   ```
   ...
   **— loop_start —** 
   ...
   sudo chown adminMe:adminMe /home/adminAnsible/.ssh/authorized_keys
   ```
...
9. 关闭root用户的ssh登录权限
   ...
   ```bash
   # [admin]
   sudo systemctl restart sshd          # 重启服务确保配置生效
   ```
...
10. 关闭其他所有用户的密码登入权限
   ...
   同理加入这两行（关闭交互式质询认证和键盘交互认证，开启PAM认证）：
   ...
   ```bash
   # [admin]
   sudo systemctl restart sshd          # 一样的重启
   ```
...
11. 配置全自动安全更新初始化
   ...
   ```
   此处为自动安全更新，dnf-automatic.timer 默认每天随机时间触发 dnf-automatic.service 检查并安装安全更新。随机时间是有意设计——避免所有服务器同时更新造成负载，同时减少被预测的窗口。
12. 为部署网页做准备
   ...
   ```bash
   # [admin]
   sudo dnf install nginx               # 安装展示网页用的软件
...
13. 配置HTTPS
   ...
   sudo vim /etc/nginx/conf.d/[项目名称].conf
   ```
   ...
   检查能否访问https的网页
14. SSR网页（全栈框架）/Node.js API（Express等）的后端部署
